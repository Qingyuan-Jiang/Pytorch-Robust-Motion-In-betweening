...
if __name__ == '__main__':
    ...
    # load train data #
    lafan_data_test = LaFan1(opt['data']['data_dir'], seq_len=opt['model']['seq_length'], offset=40, train=False,
    ...
    for epoch in range(opt['test']['num_epoch']):
        ...
        for i_batch, sampled_batch in enumerate(lafan_loader_test):
            ...
            with torch.no_grad():
                ...
                if False:
                    print('local_q:', local_q.size(), 'root_v:', root_v.size(), 'contact:', contact.size(),
                          'root_p_offset:', root_p_offset.size(), 'local_q_offset:', local_q_offset.size(), 'target:', target.size())
                    assert 0

                lstm.init_hidden(local_q.size(0))
                h_list = []
                quat_list = [local_q[:, 0, ].view(local_q.size(0), -1, 4)]
                pred_list = [X[:, 0]]
                bvh_list = [torch.cat([X[:, 0, 0], local_q[:, 0, ].view(local_q.size(0), -1)], -1)]
