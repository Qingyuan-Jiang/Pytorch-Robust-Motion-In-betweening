...
if __name__ == '__main__':
    ...
    stamp = time.strftime("%Y_%m-%d-%H_%M_%S", time.localtime(time.time()))
    ...
    if opt['train']['use_adv']:
        ...
        if len(opt['train']['pretrained']) > 0:
            short_discriminator.load_state_dict(
                torch.load(os.path.join(opt['train']['pretrained'], 'short_discriminator.pkl')))
            long_discriminator.load_state_dict(
                torch.load(os.path.join(opt['train']['pretrained'], 'long_discriminator.pkl')))
